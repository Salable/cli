...
import { ICreateFeatureQuestionAnswers, IFeature, IUpdateFeatureQuestionAnswers } from '../types';
import { processAnswers } from './process-answers';

export const planFeatureMenu = async ({ valueType, uuid, ...feat }: IFeature) => {
  ...
  switch (valueType) {
    ...
    case 'boolean':
      ...
      >(CREATE_PLAN_QUESTIONS.TRUE_FALSE_DEFAULT(valueType, feat.defaultValue, feat.name));
    ...
    case 'numerical':
      const { showUnlimited, unlimitedNumberDefault, numberDefault } =
        await processAnswers<ICreateFeatureQuestionAnswers>([
          CREATE_PLAN_QUESTIONS.NUMERICAL_SHOW_UNLIMITED(feat.showUnlimited, feat.name),
          CREATE_PLAN_QUESTIONS.NUMERICAL_UNLIMITED_NUMBER_DEFAULT(
            feat.defaultValue === '-1' ? 'Unlimited' : 'Number',
            feat.name
          ),
          CREATE_PLAN_QUESTIONS.NUMERICAL_NUMBER_DEFAULT(parseInt(feat.defaultValue), feat.name),
        ]);
    ...
    case 'enum':
      ...
      const { textOptionsDefault } = await processAnswers<ICreateFeatureQuestionAnswers>(
        CREATE_PLAN_QUESTIONS.TEXT_OPTIONS_DEFAULT(enumNames, feat.name)
      );

      return {
        enumValue:
          feat.featureEnumOptions.find(({ name }) => name === textOptionsDefault)?.uuid || '',
        featureUuid: uuid,
        isUnlimited: false,
        value: '',
      };
